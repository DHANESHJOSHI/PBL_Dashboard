...
const YOUR_EMAIL = "rohit@csrbox.org";
...
// Helper function to share team folder with anyone having the link (restricted access)
async function shareTeamFolder(drive, folderId) {
  try {
    // Create permission for anyone with the link to view/edit
    const permission = {
      role: 'writer', // Teams can edit content in their folder
      type: 'anyone'
    };
    ...
    console.log(`Team folder shared successfully: ${folderId}`);
  } catch (error) {
    console.error(`Error sharing team folder ${folderId}:`, error);
    // Don't throw error, just log it as sharing is not critical for folder creation
  }
}
...
export async function createTeamFolderStructure(drive, teamData, GlobalData) {
  return withExponentialBackoff(async () => {
    try {
      ...
      // Share the team folder with anyone having the link (restricted to this folder only)
      await shareTeamFolder(drive, teamFolderId);

      const folderStructure = {
        teamsFolderId: rootFolderId,
        teamFolderId,
        teamFolderLink: `https://drive.google.com/drive/folders/${teamFolderId}`,
        mainFolders: {},
        memberFolders: {},
        createdAt: new Date(),
      ...
      folderStructure.finalDeliverableFolderId = finalDeliverableId;
...
export async function uploadToTeamFolder(drive, teamData, fileData, submissionType, subCategory = null, memberIndex = null, GlobalData = null, isRetry = false) {
  return withExponentialBackoff(async () => {
    try {
      ...
      try {
        ...
        if ((error.code === 404 || error.status === 404) && !isRetry) {
          console.log('Target folder not found (404). Recreating folder structure...');
          teamData.folderStructure = await ensureTeamFolderStructure(drive, teamData, GlobalData);
          console.log('Retrying upload recursively with new folder structure...');
          return uploadToTeamFolder(drive, teamData, fileData, submissionType, subCategory, memberIndex, GlobalData, true);
        }
        throw error;
      }
