...
const YOUR_EMAIL = 'hgohil220@gmail.com';
...
async function findOrCreateFolder(drive, folderName, parentId = null, shareWithEmail = false) {
  return withExponentialBackoff(async () => {
    try {
      // Check if folder already exists (using your reference approach)
      ...
      // Folder does not exist so create (using your reference approach)
      ...
      // Share folder with your email for management access
      if (YOUR_EMAIL) {
        try {
          ...
          console.log(`✅ Shared folder ${folderName} with ${YOUR_EMAIL} as writer`);
        } catch (error) {
          console.warn(`⚠️ Failed to share folder with ${YOUR_EMAIL}: ${error.message}`);
        }
      }
      ...
      try {
        ...
        console.log(`Made folder ${folderName} accessible to anyone with link`);
      } catch (error) {
...
export async function createTeamFolderStructure(drive, teamData) {
  return withExponentialBackoff(async () => {
    try {
      ...
      // 1. First find the TestDrive folder in your drive
      console.log('Looking for TestDrive folder in your drive...');
      const testDriveQuery = `name='TestDrive' and mimeType='application/vnd.google-apps.folder' and trashed=false`;
      const testDriveResult = await drive.files.list({
        q: testDriveQuery,
        fields: 'files(id, name, permissions)',
        spaces: 'drive',
      });

      let testDriveFolderId;
      if (testDriveResult.data.files.length > 0) {
        testDriveFolderId = testDriveResult.data.files[0].id;
        console.log(`Found existing TestDrive folder with ID: ${testDriveFolderId}`);

        // Check if TestDrive folder is properly shared
        try {
          const permissions = await drive.permissions.list({
            fileId: testDriveFolderId,
          ...
          if (!hasPublicAccess) {
            console.log('TestDrive folder not shared publicly, making it accessible...');
            await drive.permissions.create({
              fileId: testDriveFolderId,
              resource: {
                role: 'writer',
            ...
            console.log('Made existing TestDrive folder accessible to anyone with link');
          } else {
            console.log('TestDrive folder is already properly shared');
          }
        } catch (error) {
          console.warn('Could not check/set TestDrive folder permissions:', error.message);
        }
      } else {
        // Create TestDrive folder if it doesn't exist
        console.log('TestDrive folder not found in your drive, creating it...');
        const testDriveFolder = await drive.files.create({
          resource: {
            name: 'TestDrive',
            mimeType: 'application/vnd.google-apps.folder',
          },
        ...
        testDriveFolderId = testDriveFolder.data.id;
        console.log(`Created new TestDrive folder with ID: ${testDriveFolderId}`);

        // Share TestDrive folder with your email for management access
        if (YOUR_EMAIL) {
          try {
            await drive.permissions.create({
              fileId: testDriveFolderId,
            ...
            console.log(`✅ Shared TestDrive folder with ${YOUR_EMAIL} as writer`);
          } catch (error) {
            console.warn(`⚠️ Failed to share TestDrive folder with ${YOUR_EMAIL}: ${error.message}`);
          }
        }

        // Then make it accessible to anyone with link
        try {
          await drive.permissions.create({
            fileId: testDriveFolderId,
            resource: {
          ...
          console.log('Made new TestDrive folder accessible to anyone with link');
        } catch (error) {
          console.warn('Failed to set TestDrive folder permissions:', error.message);
        }
      }

      // 2. Create main teams folder under TestDrive
      const teamsFolderId = await findOrCreateFolder(drive, folderNames.rootFolder, testDriveFolderId, true);
